    ...
    F: FnMut(&Path, &[R]),
{
    ...
    // Captured once for the whole walk, not per batch. Every record read here
    // is priced under the catalog in force when the scan began; if models.dev
    // refreshes part-way through, the remaining batches are dropped rather
    // than mixed into an index the next reader treats as current (SBS-946).
    let started = store.read().fingerprint();
    for chunk in files.chunks(batch) {
        if is_cancelled(cancel) {
            break;
        }
