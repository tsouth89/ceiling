...
- **The 1.5.33 notes no longer claim the incident badge is Ceiling's only non-provider request.** That sentence was false: models.dev (prices) and GitHub (update checks) already run without that opt-in. The badge is still off by default. The copy now says it is the only outbound request tied to your provider list and names those two hosts.
