...
- **The OpenCode mark now matches the real brand on every surface.** OpenCode's square-ring logo is monochrome, black on white or white on black, but nothing showed it that way. The taskbar strip painted it in an invented blue, and the dashboard drew it in the near-black taken straight from the brand asset, which left it almost invisible against a dark card. Both now use the same light neutral the Grok mark already uses, and the bundled SVG follows the theme instead of carrying a fixed fill. Chart bars move to the mid gray that Grok, the other monochrome brand, already uses.
