...
- **The chart cache no longer grows forever.** Its keys include the reset window they were built for, so every provider reset started a new entry and left the old one on disk for good. Someone who opens Charts after each reset was adding entries every day, and all of them were read back at startup. Entries now expire after two days, with a 256-entry ceiling as a backstop. An oversized cache from an earlier build is trimmed and rewritten on the next launch, including one left by an older cache format.
